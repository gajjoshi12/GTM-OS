...
export function ProspectsPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader eyebrow="Phases 2–4 · ICP → Discovery → Enrichment → Verification" title={<>Ideal customers, <span className="gradient-text">scored and verified</span></>}
        description="ICPs derived from data. Every account scored 0–100 across fit dimensions and tiered. Provider-agnostic waterfall enrichment; bad data purged before outbound."
        actions={<><Button variant="outline" onClick={() => enrich.mutate()} loading={enrich.isPending}><ShieldCheck className="h-4 w-4" /> Enrich & verify</Button><Button onClick={() => discover.mutate()} loading={discover.isPending}><Sparkles className="h-4 w-4" /> Discover 5,000 accounts</Button></>} />

      <div className="grid grid-cols-2 gap-3 md:grid-cols-5">
        {[['Accounts', num(stats?.total)], ['Tier 1', num(stats?.by_tier?.['1'])], ['Tier 2', num(stats?.by_tier?.['2'])], ['Avg fit score', String(stats?.avg_fit ?? '—')], ['Enriched', `${stats && stats.total ? Math.round((stats.enriched / stats.total) * 100) : 0}%`]].map(([l, v]) => (
      ...
      <div className="flex flex-wrap items-center gap-3">
        <Tabs tabs={[{ key: 'icps', label: 'ICPs', count: icps?.length }, { key: 'accounts', label: 'Accounts', count: stats?.total }, { key: 'contacts', label: 'Contacts' }]} value={tab} onChange={setTab} />
        {tab !== 'icps' && <>
          ...
          {tab === 'accounts' && <Tabs tabs={[{ key: '', label: 'All tiers' }, { key: '1', label: 'Tier 1' }, { key: '2', label: 'Tier 2' }, { key: '3', label: 'Tier 3' }]} value={tier} onChange={setTier} />}
        </>}
      </div>
        ...
        <div className="grid gap-4 xl:grid-cols-3">
          {icps?.map((icp, i) => (
            <motion.div key={icp.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className={`glass p-5 ${icp.rank === 1 ? 'ring-1 ring-accent/40' : ''}`}>
              <div className="flex items-start justify-between gap-2"><div><div className="label">ICP #{icp.rank}</div><h3 className="mt-1 text-base font-bold leading-snug text-white">{icp.name}</h3></div><Badge tone={statusTone(icp.status)}>{icp.status}</Badge></div>
              <p className="mt-2 text-xs text-[var(--text-secondary)]">{icp.description}</p>
              <div className="mt-4 grid grid-cols-3 gap-2 text-center">
                <div className="rounded-xl bg-white/[0.03] p-2"><div className="text-lg font-bold text-white">{money(icp.expected_acv, 'INR')}</div><div className="text-[10px] text-[var(--text-muted)]">Expected ACV</div></div>
                <div className="rounded-xl bg-white/[0.03] p-2"><div className="text-lg font-bold text-white">{num(icp.estimated_tam)}</div><div className="text-[10px] text-[var(--text-muted)]">TAM accounts</div></div>
              ...
              <div className="mt-4"><div className="label mb-1.5">Personas</div>{icp.personas.map((p) => <div key={p.title} className="mb-1 rounded-lg border border-white/[0.06] px-2.5 py-1.5 text-xs"><span className="font-semibold text-white">{p.title}</span><span className="text-[var(--text-muted)]"> — {p.angle}</span></div>)}</div>
              {icp.status !== 'approved' && <Button size="sm" variant="success" className="mt-4 w-full" onClick={() => approve.mutate(icp.id)}><Check className="h-3.5 w-3.5" /> Approve for outbound</Button>}
            </motion.div>
          ))}
        </div>
      )}
            ...
            <tbody>
              {companies?.results.map((c) => (
                <tr key={c.id} onClick={() => setOpen(c)} className="cursor-pointer border-t border-white/[0.05] transition hover:bg-white/[0.03]">
                  ...
                  <td className="px-3 py-3"><Badge tone={c.tier === 1 ? 'accent' : c.tier === 2 ? 'cyan' : 'neutral'}>Tier {c.tier}</Badge></td>
      ...
      <Modal open={!!open} onClose={() => setOpen(null)} width="max-w-3xl">
        {detail && (
          <div className="space-y-5">
            <div className="flex items-start gap-4"><Mark seed={detail.name} size={48} /><div className="flex-1"><h3 className="text-xl font-bold text-white">{detail.name}</h3><div className="text-xs text-[var(--text-muted)]">{detail.domain} · {detail.city}, {detail.country} · {num(detail.headcount)} staff · revenue {money(detail.revenue, 'INR')}</div><div className="mt-2 flex flex-wrap gap-1.5">{detail.tech_stack.map((t) => <Badge key={t}>{t}</Badge>)}</div></div><div className="text-right"><div className="label">Fit</div><div className="text-3xl font-bold text-white">{detail.fit_score}</div><Badge tone="accent">Tier {detail.tier}</Badge></div></div>
            <div className="grid gap-4 md:grid-cols-2">
              <div><div className="label mb-2">Score breakdown</div>{Object.entries(detail.score_breakdown).map(([k, v]) => <div key={k} className="mb-1.5"><div className="flex justify-between text-[11px]"><span className="text-[var(--text-secondary)]">{title(k)}</span><span className="num text-white">{v}</span></div><Meter value={v} height={4} /></div>)}</div>
              <div><div className="label mb-2">Buying signals</div>{detail.signals.map((s) => <div key={s} className="mb-1 rounded-lg border border-emerald-500/20 bg-emerald-500/5 px-2.5 py-1.5 text-xs text-emerald-200">{s}</div>)}</div>
            </div>
            <div><div className="label mb-2">Decision-makers · Prospect Intelligence Cards</div>
              <div className="space-y-2">{detail.contacts?.map((ct) => (
                <details key={ct.id} className="rounded-xl border border-white/[0.06] bg-white/[0.02]"><summary className="flex cursor-pointer items-center gap-3 px-3 py-2.5 text-sm"><span className="font-semibold text-white">{ct.full_name}</span><span className="text-xs text-[var(--text-muted)]">{ct.title}</span><Badge tone={statusTone(ct.email_status)} className="ml-auto">{ct.email_status}</Badge><span className="num text-xs text-white">{ct.lead_score}</span></summary>
                  <div className="grid gap-3 border-t border-white/[0.05] px-3 py-3 text-xs md:grid-cols-2">
                    <div><div className="label mb-1">Intelligence card</div>{Object.entries(ct.intelligence_card).map(([k, v]) => <div key={k} className="flex gap-2 py-0.5"><span className="w-24 shrink-0 text-[var(--text-muted)]">{title(k)}</span><span className="text-white">{v}</span></div>)}</div>
                    <div><div className="label mb-1"><Database className="mr-1 inline h-3 w-3" />Waterfall enrichment</div>{ct.enrichment_log.map((e, i) => <div key={i} className="flex items-center gap-2 py-0.5"><span className={e.ok ? 'text-emerald-400' : 'text-rose-400'}>{e.ok ? '✓' : '✗'}</span><span className="text-white">{e.provider}</span><span className="text-[var(--text-muted)]">{e.fields.join(', ')}</span></div>)}
                      <div className="label mb-1 mt-3">Verification</div>{Object.entries(ct.verification).filter(([k]) => k !== 'checked_at').map(([k, v]) => <div key={k} className="flex gap-2 py-0.5"><span className="w-24 text-[var(--text-muted)]">{title(k)}</span><span className={v === true ? 'text-emerald-300' : v === false ? 'text-[var(--text-secondary)]' : 'text-white'}>{String(v)}</span></div>)}</div>
                  </div></details>))}</div></div>
          </div>
        )}
      </Modal>
