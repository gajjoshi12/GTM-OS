...
export function SettingsPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={<>You set the limits. <span className="gradient-text">The AI works inside them.</span></>}
        description="How much the AI can do on its own, how much it is allowed to spend, and the writing rules it has to follow. Change any of it at any time."
        actions={<Tabs tabs={[{ key: 'control', label: 'What the AI can do' }, { key: 'brand', label: 'How it should write' }, { key: 'business', label: 'About your business' }]} value={tab} onChange={setTab} />} />
      ...
      {tab === 'control' && c && (
        <div className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Panel title="Spending limits" subtitle="Hard caps the AI can never go past">
            ...
            <Panel title="Permissions" subtitle="Turn on only what you are comfortable with">
          ...
          <Panel title="Per-agent exceptions" subtitle="For example: let the reply-handler act alone, but always approve ad spend yourself.">
      ...
      {tab === 'brand' && b && (
        <div className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            <Panel title="Voice and tone" subtitle="Applied to every email, ad and post the AI writes">
            ...
            <Panel title="What it may and may not claim" subtitle="Checked before anything goes out">
              <div className="space-y-3">
      ...
      {tab === 'business' && biz && (
        <div className="space-y-4">
          <Panel title="Your business in one sentence" subtitle="Everything the AI plans starts from this">
          ...
          <Panel title="The plan the AI built from it" subtitle="Regenerate after you change the sentence above"><pre className="max-h-96 overflow-auto rounded-xl bg-ink-950/70 p-4 font-mono text-[11px] leading-relaxed text-[var(--text-secondary)]">{JSON.stringify(biz.derived_plan, null, 2)}</pre></Panel>
        </div>
      )}
    </div>
  )
