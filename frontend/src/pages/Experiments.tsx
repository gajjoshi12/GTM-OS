...
export function ExperimentsPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader eyebrow="Phase 13 · Experimentation Engine → Marketing Memory" title={<>Every action is a hypothesis. <span className="gradient-text">Every result is remembered.</span></>}
        description="Hypothesis → Experiment → Execution → Measurement → Statistical confidence → Decision → Learning. Insights propagate automatically into email, ads, pages, LinkedIn and sales scripts."
        actions={<><Tabs tabs={[{ key: 'experiments', label: 'Experiments' }, { key: 'memory', label: 'Marketing Memory', count: memory?.length }]} value={tab} onChange={setTab} /><Button onClick={() => propose.mutate()} loading={propose.isPending}><Sparkles className="h-4 w-4" /> Propose experiments</Button></>} />

      <div className="grid grid-cols-2 gap-3 md:grid-cols-5">
        {[['Running', stats?.running], ['Proposed', stats?.proposed], ['Concluded', stats?.concluded], ['Winners', stats?.winners], ['Avg lift', `${stats?.avg_lift ?? 0}%`]].map(([l, v]) => <div key={String(l)} className="glass p-4"><div className="label">{l}</div><div className="mt-1 text-2xl font-bold text-white">{v ?? '—'}</div></div>)}
      </div>

      {tab === 'experiments' && (
        <>
          <Tabs tabs={[{ key: '', label: 'All' }, { key: 'running', label: 'Running' }, { key: 'concluded', label: 'Concluded' }, { key: 'proposed', label: 'Backlog' }]} value={status} onChange={setStatus} />
            ...
            {exps?.map((e, i) => {
              ...
              return (
                <motion.div key={e.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }} className="glass p-5">
                  ...
                  <p className="mt-2 text-xs italic text-[var(--text-secondary)]">H: {e.hypothesis}</p>
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-3 text-[11px] text-[var(--text-muted)]">
                    <span>Confidence <b className="num text-white">{Math.round(e.confidence * 100)}%</b></span>
                    {e.lift_pct > 0 && <span>Lift <b className="num text-emerald-300">+{e.lift_pct}%</b></span>}
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {memory?.map((m, i) => (
            <motion.div key={m.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }} className="glass relative overflow-hidden p-5">
              ...
              <div className="flex items-center gap-2"><Badge tone="neutral">{title(m.category)}</Badge><span className="text-[11px] text-[var(--text-muted)]">{Math.round(m.confidence * 100)}% confidence · applied {m.times_applied}×</span><span className="ml-auto num text-xs font-bold text-white">impact {m.impact_score}</span></div>
